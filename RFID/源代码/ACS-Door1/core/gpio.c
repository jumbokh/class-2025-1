...
volatile uint32_t p1_8_counter  = 0;
volatile uint32_t p2_4_counter  = 0;
volatile uint32_t p3_1_counter  = 0;

static uint8_t _gpioInitialised = false;
void wg_isr_handler ( int bitVal );
...
void PIOINT2_IRQHandler ( void )
{
    ...
    gpio2_counter++;
	/*p2.4的中断状态*/
    regVal = gpioIntStatus ( 2, 4 );
    if ( regVal )
    {
        p2_4_counter++;
        wg_isr_handler ( 0 ); // bit 0 is received
        gpioIntClear ( 2, 4 );
...
void gpioSetInterrupt ( uint32_t portNum, uint32_t bitPos, gpioInterruptSense_t sense, gpioInterruptEdge_t edge, gpioInterruptEvent_t event )
{
    ...
    if ( gpioInterruptSense_Edge == sense )
    {
        *gpiois &= ~ ( 0x1 << bitPos );
        /* single or double only applies when sense is 0(edge trigger). */
        ( gpioInterruptEdge_Single == edge ) ? ( *gpioibe &= ~ ( 0x1 << bitPos ) ) : ( *gpioibe |= ( 0x1 << bitPos ) );
    }
    else
    {
        *gpiois |= ( 0x1 << bitPos );
    ...
    ( gpioInterruptEvent_ActiveHigh == event ) ? ( *gpioiev &= ~ ( 0x1 << bitPos ) ) : ( *gpioiev |= ( 0x1 << bitPos ) );
...
*/
/**************************************************************************/
void gpioSetPullup ( volatile REG32 *ioconReg, gpioPullupMode_t mode )
{
    if ( !_gpioInitialised ) gpioInit();
